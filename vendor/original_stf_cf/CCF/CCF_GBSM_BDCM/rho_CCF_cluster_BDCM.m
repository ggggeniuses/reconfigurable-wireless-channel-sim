function rho_CCF_NLoS_cluster_rng_nor = rho_CCF_cluster_BDCM(j, lambda, v_T, v_R, eta_ver_T, eta_azi_T, ...
...
rho_CCF_NLoS_cluster_rng = zeros(1, length(delta_q));
rho_CCF_NLoS_cluster_rng_nor = zeros(1, length(delta_q));
for id_1 = 1 : length(delta_q)   

    q_q = q + delta_q(id_1);
    
    rho_CCF_NLoS_cluster_same_delta_q = 0;
    Angle_PDF = 0;
    for alpha_R_n = AAoA_low : AAoA_step : AAoA_up             % sum with AAoA
        for beta_R_n = EAoA_low : EAoA_step : EAoA_up          % sum with EAoA
            ...
            [xi_T_n_t, xi_R_n_t, nu_t, theta_ver_T_n_t, theta_azi_T_n_t, theta_ver_R_n_t, theta_azi_R_n_t] = rho_CCF_cluster_time_BDCM(v_T, v_R,...
                                                                 eta_ver_T, eta_azi_T, eta_azi_R, t, d_n, D_0, H_0, P, Q, p, q, delta_T, delta_R, ...
                                                                 psi_ver_T, psi_azi_T, psi_ver_R, psi_azi_R, lambda);
            
            [xi_T_n_tt, xi_R_n_tt, nu_tt, theta_ver_T_n_tt, theta_azi_T_n_tt, theta_ver_R_n_tt, theta_azi_R_n_tt] = rho_CCF_cluster_time_BDCM(v_T, v_R, ...
                                                    eta_ver_T, eta_azi_T, eta_azi_R, t+tt, d_n, D_0, H_0, P, Q, p_p, q_q, delta_T, delta_R, ...
                                                    psi_ver_T, psi_azi_T, psi_ver_R, psi_azi_R, lambda);
            ...
            rho_CCF_NLoS_cluster_same_delta_q = rho_CCF_NLoS_cluster_same_delta_q ...
                               + 1/(P*Q) * exp(j*(2*pi)/lambda * (nu_t - nu_tt))...
    ...
    rho_CCF_NLoS_cluster_rng(id_1) = rho_CCF_NLoS_cluster_same_delta_q/Angle_PDF;
    if id_1 == 1         % used for normalization
        rho_CCF_NLoS_nor = rho_CCF_NLoS_cluster_rng(id_1);
    end
    rho_CCF_NLoS_cluster_rng_nor(id_1) = rho_CCF_NLoS_cluster_rng(id_1)/rho_CCF_NLoS_nor;
